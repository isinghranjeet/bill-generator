...
//                 rows={2}
//               />
//               <div className="hidden print:block border border-gray-300 p-2 rounded bg-gray-50 print:bg-gray-50 print:print-color-adjust-exact min-h-[50px] text-xs">
//                 {remarks || "No remarks"}
//               </div>
//             </>
//           ) : (
//             <div className="border border-gray-300 p-2 rounded bg-gray-50 print:bg-gray-50 print:print-color-adjust-exact min-h-[50px] text-xs">
//               {remarks || "No remarks"}
//             </div>
//           )}
//         </div>
...
export const ProfessionalInvoice = React.memo(({ 
  ...
  const documentType: DocumentType = useMemo(() => {
    ...
    return 'invoice';
  }, [documentTypeOverride, details.quotationNo, details.invoiceNo]);
          ...
          <div className="flex justify-between items-start print:flex-nowrap">
            ...
            <div className="border-l pl-3 ml-3 min-w-[200px] print:min-w-[140px] print:pl-2 print:ml-2">
              ...
              <div className="space-y-1 print:space-y-0.5">
                ...
                {editable && onDetailsChange ? (
                  <div className="grid grid-cols-2 gap-1 items-center print:hidden">
                    <span className="font-semibold text-xs">Document Type:</span>
                    <Select
                      value={documentType}
                      onValueChange={(value: DocumentType) => handleDocumentTypeChange(value)}
                    >
                ...
                {/* Invoice Number - Searchable Dropdown (shown when document type is invoice) */}
                  ...
                  <div className="grid grid-cols-2 gap-1 items-center print:grid-cols-2 print:gap-0.5">
                    ...
                    {editable && onDetailsChange ? (
                      <>
                        ...
                        {/* Print-only fallback: the Popover/Command UI above
                            is interactive-only and never appears in print or
                            PDF, so the stored value is rendered as plain text
                            here — this is what fixes the "blank in print" bug. */}
                        <span className="hidden print:block text-xs print:text-[7px] font-medium">
                          {details.invoiceNo || "-"}
                        </span>
                      </>
                    ) : (
                      <span className="font-medium text-xs print:text-[7px] print:font-normal">{details.invoiceNo || "-"}</span>
                    )}
                  </div>
                )}

                {/* Quotation Number - Simple Text Input (shown when document type is quotation) */}
